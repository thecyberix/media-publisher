...
WhisperX alignment models (~360 MB English wav2vec2) are **not** installed on the CI test matrix. Warm them with **Actions → Warm WhisperX align cache**. That job uses a **separate pip cache** (`pip-align-…`) from the CI test cache, installs CPU Torch 2.8 (the version WhisperX requires) from `download.pytorch.org`, then saves `TORCH_HOME` / `HF_HOME` under `align-models-…`. A later alignment job should use the same setup-python-env inputs (`extras: align`, `cache-align: true`, `torch-cpu: true`).

GitHub **does not update** an existing `actions/cache` / `setup-python` pip entry when the key already exists. CI had already created the default pip cache from `pyproject.toml` *without* Torch, so the warmup job restored that key and then **skipped saving** the 800+ MB CUDA wheel it downloaded. The dedicated `pip-align-` key avoids that. Also do not install “latest CPU Torch”: WhisperX wants `torch~=2.8.0`, so Torch 2.13+cpu gets uninstalled and replaced by PyPI’s CUDA 2.8 wheel.
